...
export const rook = (rank: number, file: number): movesT => {
  const moves = [
    indices.map(x => addN(rank, file - x)(x)).filter(x => x.every(validIndex)),
    indices.map(x => subN(rank, file + x)(x)).filter(x => x.every(validIndex)),
    indices.map(x => addN(rank - x, file)(x)).filter(x => x.every(validIndex)),
    indices.map(x => subN(rank + x, file)(x)).filter(x => x.every(validIndex)),
  ]
  return moves.flatMap(x => x)
}
